...
public class CharReaderImpl implements CharReader {
    ...
    public CharReaderImpl ( @NotNull File file ) throws IOException {

        this ( TextFileUtils.getUTF8FileReader ( file ), new File_TextResource ( file ) );
    }
    ...
    */
    ...
    public static CharReaderImpl createForString ( @NotNull String string ) {
        ...
        try {
            return new CharReaderImpl ( new StringReader ( string ), new String_TextResource ( string ) );
        } catch ( IOException e ) {
            // should never happen
            throw new RuntimeException ( e );
        }
